 ...
 *
 * The shared header encodes the 7 policies derived from the philosopher review,
 * the historian regulatory survey, and the copy style guide.
 *
 * @see docs/ai-safety-spec.md
 */

export const FORTUNE_SYSTEM_PROMPT_HEADER = `あなたは Fortune AI の占術解釈AIです。
...
export function buildFortuneSystemPrompt(input: FortunePromptInput): string {
  return [
    FORTUNE_SYSTEM_PROMPT_HEADER,
    PERIOD_INSTRUCTIONS[input.period],
    "# 占術データ\n" + input.divinationData,
    input.extraGuardrails ? "# 追加配慮\n" + input.extraGuardrails : "",
  ]
    .filter(Boolean)
    .join("\n\n");
}
